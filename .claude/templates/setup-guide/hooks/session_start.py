...
def get_semantic_stats(project: str) -> dict:
    ...
    try:
        result = subprocess.run(
            ["python3", str(SEMANTIC_SCRIPT), "stats", project],
...
def main():
    ...
    if memory_index:
        ...
        mem_summary = f"🧠 {total_memories} memories (~{total_tokens} tokens)"
        if high_priority:
    ...
    if recent_obs:
        msg_parts.append(f"📝 {len(recent_obs)} recent observations")
    ...
    if total_skills > 0:
        ...
        msg_parts.append(f"🛠️ Skills: {', '.join(skill_parts)}")
    ...
    if beads_status.get("initialized"):
        ...
        if total > 0:
            msg_parts.append(f"📋 Beads: {open_count} open, {ready} ready")
        else:
            msg_parts.append("📋 Beads: initialized")

    # Session continuation available
    if source == "startup":
        last_session = cmd(f"python3 {MEMORY_SCRIPT} get-last-session '{project}'") if MEMORY_SCRIPT.exists() else ""
        if last_session:
            msg_parts.append("💾 /session-continue available")
