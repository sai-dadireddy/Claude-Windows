...
def save_memory(project: str, memory_type: str, content: str):
    ...
    try:
        result = subprocess.run(
            [sys.executable, str(MEMORY_SCRIPT), "save-memory", project, memory_type, content],
...
def main():
    ...
    if should_remind_to_save(activity):
        ...
        reminder_output = {
            "systemMessage": f"[SAVE] Big session! {summary} - Consider saving important work with memory system."
        }
