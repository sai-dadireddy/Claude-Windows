#!/usr/bin/env python3
"""
Electron Test Generator for Advanced Compensation
...
OUTPUT_DIR = r"C:\Users\SainathreddyDadiredd\OneDrive - ERPA\Claude\workday_docs\electron_tests\Advanced_Compensation"
...
def calculate_confidence(scenario, rag_result):
    ...
    if any(verb in task.lower() for verb in ['create', 'submit', 'configure', 'review', 'approve', 'calculate']):
        score += 1.0

    # Has business object reference (1 point)
    if any(obj in task.lower() for obj in ['compensation', 'bonus', 'plan', 'award', 'grant', 'incentive', 'equity']):
        score += 1.0
...
def generate_electron_steps(scenario, rag_result):
    ...
    if rag_result:
        if 'create' in task_step.lower() and ('plan' in task_step.lower() or 'bonus' in task_step.lower()):
            steps.append('4. wait for compensation page')
            steps.append('5. enter compensation plan details')
            steps.append('6. configure plan rules')
            steps.append('7. click "Submit" button')
        elif 'award' in task_step.lower() or 'grant' in task_step.lower():
            steps.append('4. wait for page load')
            steps.append('5. select employees for award')
            steps.append('6. enter award amounts')
            steps.append('7. click "Submit" button')
        elif 'calculate' in task_step.lower() or 'process' in task_step.lower():
            steps.append('4. wait for calculation page')
            steps.append('5. select calculation period')
            steps.append('6. verify compensation calculations')
            steps.append('7. click "Process" or "Submit" button')
        elif 'approve' in task_step.lower() or 'review' in task_step.lower():
            steps.append('4. wait for inbox page')
            steps.append('5. click compensation item to review')
            steps.append('6. verify award details')
            steps.append('7. click "Approve" or "Submit" button')
        else:
            steps.append('4. wait for page load')
...
def generate_test_file(scenario, rag_result, output_path):
    ...
    content.append("FUNCTIONAL AREA: Advanced Compensation")
...
def main():
    ...
    print("=" * 80)
    print("WORKDAY ELECTRON TEST GENERATOR - ADVANCED COMPENSATION")
    print("=" * 80)
    print()
    ...
    df = pd.read_excel(EXCEL_PATH, sheet_name='Advanced_Compensation')
    print(f"Total Advanced Compensation scenarios: {len(df)}")
    print()
