#!/usr/bin/env python3
"""
Electron Test Generator for Expenses
...
OUTPUT_DIR = r"C:\Users\SainathreddyDadiredd\OneDrive - ERPA\Claude\workday_docs\electron_tests\Expenses"
...
def calculate_confidence(scenario, rag_result):
    ...
    if any(verb in task.lower() for verb in ['create', 'submit', 'approve', 'review', 'expense', 'report']):
        score += 1.0

    # Has business object reference (1 point)
    if any(obj in task.lower() for obj in ['expense', 'report', 'reimbursement', 'travel', 'receipt']):
        score += 1.0
...
def generate_electron_steps(scenario, rag_result):
    ...
    if rag_result:
        if 'create' in task_step.lower() or 'expense report' in task_step.lower():
            steps.append('4. wait for page load')
            steps.append('5. enter expense details')
            steps.append('6. attach receipt (if required)')
            ...
            steps.append('5. click expense report to review')
            steps.append('6. verify expense details')
            steps.append('7. click "Approve" or "Submit" button')
        elif 'reimburse' in task_step.lower() or 'payment' in task_step.lower():
            steps.append('4. wait for page load')
            steps.append('5. verify reimbursement amount')
            steps.append('6. select payment method')
            steps.append('7. click "Submit" button')
        else:
            steps.append('4. wait for page load')
...
def generate_test_file(scenario, rag_result, output_path):
    ...
    content.append("FUNCTIONAL AREA: Expenses")
...
def main():
    ...
    print("=" * 80)
    print("WORKDAY ELECTRON TEST GENERATOR - EXPENSES")
    print("=" * 80)
    print()
    ...
    df = pd.read_excel(EXCEL_PATH, sheet_name='Expenses')
    print(f"Total Expenses scenarios: {len(df)}")
    print()
