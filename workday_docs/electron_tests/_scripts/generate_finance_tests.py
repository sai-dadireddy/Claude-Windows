#!/usr/bin/env python3
"""
Electron Test Generator for Accounting & Finance
...
def generate_test_file(scenario, rag_result, output_path):
    ...
    content.append("FUNCTIONAL AREA: Accounting & Finance")
...
def main():
    ...
    print(f"Total scenarios in Excel: {len(df)}")

    # Filter for Accounting & Finance
    finance_df = df[df['Functional Area'] == 'Accounting & Finance'].copy()
    print(f"Accounting & Finance scenarios: {len(finance_df)}")
    print()
    ...
    for idx, row in finance_df.iterrows():
        ...
        print(f"Processing [{idx+1}/{len(finance_df)}]: {scenario_id} - {scenario_name[:50]}...")
