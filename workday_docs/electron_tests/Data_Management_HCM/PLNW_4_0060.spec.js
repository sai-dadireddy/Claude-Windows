/**
 * Test: Run Tasks - Plan Data
 * Scenario ID: PLNW-4-0060
...
test.describe('PLN - Data Management (HCM) - PLNW-4-0060', () => {
  let api;
  ...
  test('Run Tasks - Plan Data', async () => {
    test.skip(true, 'Manual testing required - no task/step defined');

    // TEST SETUP
    const testData = {
      scenarioId: 'PLNW-4-0060',
      timestamp: new Date().toISOString()
    };

    // MANUAL TEST STEPS:
    // Task: -Go to Integration > Run Tasks
-Run all tasks related to the import of plan data
-Confirm that the task successfully completes at %100
-Confirm that the plan data is correct for the time period the task was run for.
    // Step: Not specified
    //
