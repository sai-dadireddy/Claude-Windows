#!/usr/bin/env python3
"""
Electron Test Generator for Tax
...
OUTPUT_DIR = r"C:\Users\SainathreddyDadiredd\OneDrive - ERPA\Claude\workday_docs\electron_tests\Tax"
...
def calculate_confidence(scenario, rag_result):
    ...
    if any(verb in task.lower() for verb in ['create', 'submit', 'configure', 'review', 'file', 'calculate']):
        score += 1.0

    # Has business object reference (1 point)
    if any(obj in task.lower() for obj in ['tax', 'form', 'withholding', 'filing', 'reporting', 'w-2', '1099']):
        score += 1.0
...
def generate_electron_steps(scenario, rag_result):
    ...
    if rag_result:
        if 'configure' in task_step.lower() or 'setup' in task_step.lower():
            steps.append('4. wait for configuration page')
            steps.append('5. enter tax configuration details')
            steps.append('6. verify tax rules')
            steps.append('7. click "Submit" or "Save" button')
        elif 'file' in task_step.lower() or 'report' in task_step.lower():
            steps.append('4. wait for report page')
            steps.append('5. select reporting period')
            steps.append('6. verify tax calculations')
            steps.append('7. click "Generate" or "Submit" button')
        elif 'calculate' in task_step.lower() or 'withholding' in task_step.lower():
            steps.append('4. wait for page load')
            steps.append('5. enter calculation parameters')
            steps.append('6. verify withholding amounts')
            steps.append('7. click "Calculate" or "Submit" button')
        else:
            steps.append('4. wait for page load')
...
def generate_test_file(scenario, rag_result, output_path):
    ...
    content.append("FUNCTIONAL AREA: Tax")
...
def main():
    ...
    print("=" * 80)
    print("WORKDAY ELECTRON TEST GENERATOR - TAX")
    print("=" * 80)
    print()
    ...
    df = pd.read_excel(EXCEL_PATH, sheet_name='Tax')
    print(f"Total Tax scenarios: {len(df)}")
    print()
